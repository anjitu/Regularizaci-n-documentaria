...
subregion = col2.selectbox("🗺️ SUB.REGÍON", ["Todas"] + sorted(df_temp["SUB.REGÍON"].dropna().unique()), key="subregion_t")
df_temp = df_temp[df_temp["SUB.REGÍON"] == subregion] if subregion != "Todas" else df_temp
...
subregion_g = colg2.selectbox("🗺️ SUB.REGÍON (Gráfico)", ["Todas"] + sorted(df_temp_g["SUB.REGÍON"].dropna().unique()), key="subregion_g")
df_temp_g = df_temp_g[df_temp_g["SUB.REGÍON"] == subregion_g] if subregion_g != "Todas" else df_temp_g
