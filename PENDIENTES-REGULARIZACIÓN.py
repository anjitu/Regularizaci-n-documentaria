...
# --- 8. Función para exportar Excel filtrado ---
...
# --- 9. Descargar pendientes filtrados ---
...
# --- 10. Función para exportar evolución de pendientes ---
def to_excel_bytes_evolutivo(df_evo):
    output = BytesIO()
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        df_evo.to_excel(writer, index=False, sheet_name='Evolución_Pendientes')
        workbook = writer.book
        worksheet = writer.sheets['Evolución_Pendientes']
        ...
        for col_num, column_name in enumerate(df_evo.columns):
            worksheet.write(0, col_num, column_name, header_format)

        for i, col in enumerate(df_evo.columns):
            col_width = max(df_evo[col].astype(str).map(len).max(), len(col)) + 2
            worksheet.set_column(i, i, col_width)

        worksheet.freeze_panes(1, 0)
        worksheet.autofilter(0, 0, df_evo.shape[0], df_evo.shape[1] - 1)

    return output.getvalue()

# --- 11. Botón para descargar evolución de pendientes ---
if not df_evolutivo.empty:
    excel_evolutivo = to_excel_bytes_evolutivo(df_evolutivo)
    st.download_button(
